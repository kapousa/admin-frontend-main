...
function CompanyForm({ username, password }) {
    ...
    const [company, setCompany] = useState({
        name: '', category: '', size: '', location: '', description: '', website: '', revenue: '', founded: '', headquarters: '', mission: '', company_values: '', financialStatement: [{ key: '', value: '' }], investors: [{ key: '', value: '' }], assessment: [{ key: '', value: '' }], portfolio: [{ key: '', value: '' }], transformation_plan: [{ key: '', value: '' }], logoFile: null,
    });
    ...
    const jsonFields = [{ name: 'financialStatement', label: 'Financial Statement' }, { name: 'transformation_plan', label: 'Assesments' }, { name: 'portfolio', label: 'Portfolio' }, { name: 'investors', label: 'Investors' }, { name: 'assessment', label: 'Assessment' }];
...
function Step5({ company, setCompany, logoPreview, setLogoPreview, handleLogoChange, jsonFields, handleFileChange, handleTopicChange, handleJsonChange, addJsonField, removeJsonField }) {
    return (
        <Grid container spacing={3}>
            <Grid item xs={12} sm={6}>
...
function Step6({ dynamicSections, setDynamicSections, handleDynamicFileChange, handleDynamicTopicChange, handleDynamicSectionChange, addDynamicItem, removeDynamicItem, addDynamicSection, removeDynamicSection }) {
